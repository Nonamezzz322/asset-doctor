...
describe('RULE_GROUP / RULES_IN_GROUP / ALL_RULES — an exhaustive, disjoint partition of the Rule union', () => {
  it('maps EXACTLY the 21 rules, each to exactly one group (a partition — no gaps, no overlaps)', () => {
    ...
    expect(new Set(ALL_RULES).size).toBe(21);
  });
  ...
  it('the humane grouping is the agreed 5/8/5/3 split (locks accidental re-homing)', () => {
    expect(GROUP_ORDER).toEqual(['integrity', 'savings', 'packing', 'vram']);
    expect(RULES_IN_GROUP.integrity).toHaveLength(5);
