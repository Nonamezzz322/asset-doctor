...
- **Доступность (a11y)** — единое token-driven кольцо фокуса `:focus-visible` по всему хрому (teal на свету, film-soft на тёмной плёнке; контраст проверен); расшифрованная легенда оверлеев + `role=img`/`aria-label` на canvas-герое (цвет больше не единственный сигнал); `aria-live`-объявления прогресса анализа, готовности результата (instant-wow ≤10s для скринридеров), счётчика и ошибок (`role=alert`) — озвучиваются только числа уже на экране, без VRAM/фейков. UX-логика вынесена в чистые Node-тестируемые модули (`focus-ring`/`film-legend`/`announce`).
...
*Обновлено 2026-06-29 (раунд 29 + UX-раунд 1). Ветка `feat/asset-pipeline` (= локальный `main`), ~68 коммитов поверх `origin/main`, всё зелёное. Деплой (GH Pages) ждёт пользовательского `git push origin main`; живые бэкенд-операции требуют бинарей toktx/pngquant/vips в задеплоенном сайдкаре.*
