...
*Обновлено 2026-06-30 (раунд 29 + UX-раунды 1-3 + сборщик ассетов R1-5). Ветка `feat/asset-pipeline` (= локальный `main`), ~81 коммит поверх `origin/main`, всё зелёное. Деплой (GH Pages) ждёт пользовательского `git push origin main`; живые бэкенд-операции требуют бинарей toktx/pngquant/vips в задеплоенном сайдкаре.*
