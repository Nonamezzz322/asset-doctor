...
## correlate R3 (upload-hitch) — честное static-evidence, без выдуманных «крупных текстур» — 2026-08-11
`13234ce`. Аудит модуля-моста `correlate` (static × runtime → вердикт): вердикт R3 upload-hitch срабатывает по
runtime-сигналу (загрузки текстур во время геймплея). Когда его подкрепляет находка dimensions-oversize — честно
цитирует «в сборке есть оверсайз-текстуры». Но БЕЗ size-находки он откатывался на «крупные текстуры в сборке» —
утверждая статический факт размера, который никто НЕ измерял (загрузки в середине игры могут быть МЕЛКИМИ
текстурами, подгруженными поздно). Его собратья runtime-only R4 (shader-hitch) и R5 (redundant-state) используют
«—» для отсутствующей статической стороны; R3 был единственным, кто фабриковал статическое утверждение. Теперь R3
цитирует «—» при отсутствии oversize-находки (runtime-only наблюдение, ровно как R4/R5) и ставит
`staticVariant:'oversize'` только когда находка это подкрепляет. Сброс staticVariant заставляет renderCorrelated
откатиться на запечённое «—» (шаблон static_none не нужен); ключ каталога static_large просто больше не
используется. Evidence-backed oversize-ветка не тронута ⇒ её локализованный вывод и renderCorrelated-drift-гард
байт-идентичны. Тесты: без size-находки ⇒ staticEvidence «—» и нет staticVariant; oversize ⇒ «oversized» +
`staticVariant:'oversize'`. Gate: typecheck + весь корпус (correlate 38) + lint.

## atlas-merge рекомендует слияние только СЛИВАЕМЫХ типов атласов (исключены Spine + BMFont) — 2026-07-29
`a530bb2`. `atlasMergeFinding` получал КАЖДЫЙ атлас и группировал все недозаполненные — но слить можно только
TexturePacker/Pixi-атласы: Pro-фикс компонует их спрайты в ОДИН TP-JSON-лист. Spine-атлас (скелет ссылается на
кадры по имени в `.atlas`) и BMFont-глиф-страница (`.fnt` ссылается на глифы) НЕ могут войти в TP-JSON-мердж,
поэтому фикс отказывает Spine-листу и пропускает шрифт насквозь. ⇒ Диагноз рекомендовал слияния, которые фикс
никогда не выполнит — и завышал мердж: напр. один TP + два недозаполненных Spine-листа давали находку «слить эти
3», хотя фикс может тронуть только один TP-лист (что даже не мердж). Пользователь только-диагноза не видит скип
фикса ⇒ рекомендация была фантомным обещанием. `atlasMergeFinding` теперь учитывает только сливаемые типы
(`texturepacker-hash` / `texturepacker-array` / `pixi`), так что рекомендация перечисляет ровно те атласы, которые
фикс скомпоновал бы. Так как planFix строит мердж-оп из refs этой находки, Spine/BMFont больше вообще не попадают
в мердж-оп (Spine-гард воркера остаётся defense-in-depth). All-TP-папки (все фикстуры) байт-идентичны. Это
диагноз/фикс-собрат scale-гарда мерджа, и БОЛЕЕ частый (Spine-листы и шрифт-страницы обычны; недозаполненные —
рутина). Тесты: 2 TP + 1 Spine + 1 BMFont ⇒ рекомендует только 2 TP; 1 TP + 2 Spine ⇒ находки нет. Gate: typecheck
+ весь корпус (analysis 272) + lint + e2e 11/11.

## atlas-merge-находка scale-осознанна — рекомендует слияние только атласов с ОДНИМ meta.scale — 2026-07-29
`3177be0`. Pro-фикс теперь отказывается сливать атласы с разным `meta.scale` (mixed-resolution лист рендерит
чужой масштаб неверно). Но ДИАГНОЗ всё ещё группировал ВСЕ недозаполненные атласы вместе независимо от scale ⇒
пользователь только-диагноза (кто не запускает фикс и не видит его скип) получал рекомендацию слить листы, которые
фикс откажется сливать — а оценка мерджа завышала достижимую экономию. `atlasMergeFinding` теперь партиционирует
недозаполненный набор по `meta.scale` и рассматривает только НАИБОЛЬШУЮ same-scale-группу — ровно тот набор,
который фикс реально слил бы. Обычный all-1× случай (scale undefined) — одна группа ⇒ `under` === весь набор ⇒
находка байт-идентична (голдены + renderFinding-drift-гард не тронуты). Сужается только реально mixed-scale набор.
Закрывает разрыв согласованности диагноз/фикс, открытый scale-гардом мерджа: рекомендация теперь совпадает с тем,
что делает фикс. Тесты: два 1× + один 0.5× недозаполненных ⇒ рекомендует только два 1×; один 1× + один 0.5×
(ни одна группа не достигает minAtlases) ⇒ находки нет. Gate: typecheck + весь корпус (analysis 271) + lint +
e2e 11/11 (all-1× фикстуры не затронуты).

## Мердж ОТКАЗЫВАЕТСЯ сливать атласы с разным meta.scale (без тихого mixed-resolution листа) — 2026-07-29
`45015e0`. `atlas-merge` группирует недозаполненные атласы ЧИСТО по occupancy (folder.ts), поэтому может выбрать
листы, экспортированные при РАЗНОМ `meta.scale` — 0.5×-вариант разрешения и 1×. Aggressive-мердж сливает группу
на ОДИН лист с ОДНИМ `meta.scale` (atlases[0]) ⇒ спрайты других масштабов рендерились бы неверного экранного
размера: mixed-resolution лист, который лоадер не может связно интерпретировать. (Проброс scale прошлого раунда
сделал выбранный scale видимым; до него мердж ронял scale — что тоже неверно для 0.5×-спрайтов.) Ветка мерджа в
воркере теперь ОТКАЗЫВАЕТ группе, где атласы не разделяют один `meta.scale`, surfaced как честный скип рядом с
существующим гардом коллизии имён спрайтов — атласы уходят нетронутыми вместо невалидного листа. Решение — чистый
экспортируемый `atlasesShareScale(atlases)` (все-undefined = все 1× = разделяют scale ⇒ обычный мердж не
затронут). Скипается только реально mixed-scale-группа; ложных срабатываний нет. Тесты: `atlasesShareScale` даёт
true для all-1x / all-0.5x / пусто / один и false для 0.5×+1× (в любом порядке) и 0.5×+0.25×. Gate: typecheck +
весь корпус (fix 541) + lint + e2e 11/11 (сценарий 10 мердж не затронут — tp-merge разделяет scale).
