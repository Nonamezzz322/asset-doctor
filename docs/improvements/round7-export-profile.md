...
**AB-R3 lifted scope — SINGLE-format prebuilt atlases now forced too.** The §5d-bis loose pass left ONE residual: a PREBUILT atlas under a SINGLE-format format-only profile was transcoded to the chosen format only when its PAGE earned a `format` finding (the prebuilt-atlas passthrough transcode op). A page already in the target format, near-optimal, or sub-threshold passed through VERBATIM — so picking "AVIF, single tier" silently produced a MIXED-format folder. AB-R3 adds a dedicated atlas-force pass after the loose pass, gated IDENTICALLY plus the single-format guard (`profileOn && !profileHasLowerTier && !profileMulti`): for each merged atlas not already claimed (`replaced`/`dropped`/`profileOwned`), it computes the single target via `resolveProfile(ref).formats[0].format`, asks the PURE `atlasNeedsForcedFormat` predicate (`packages/fix/src/atlasProfileForce.ts`, exhaustively Node-tested — lifts the inline passthrough gate logic so worker + test share ONE decision), and on `force:true` re-encodes the page through the SHARED `forceAtlasFormat` helper (the SAME body the finding-driven op handler now calls — no drift). It keeps-original-on-size-loss (never ships a larger page under a fix banner, inv 3/5), renames the page, repoints + re-emits the sidecar, drops the old page, records the variant/changes/KTX2 candidate/ops line, and increments `profileAssets`/`profileFilesEmitted`/`profileOwned`. `predictOwnerFinalNames` is extended so a force-transcoded OWNER atlas predicts `renamedTo(path, formats[0])` (divergence-safe: a size-loss bail keeps the consumer via the existing K8 machinery). STILL OUT OF SCOPE (unchanged): multi-format atlas fan-out (one page can't span N sidecars — refused at the multi-format guard), multi-tier (the tier loop already forces atlases), multi-page Spine (refused). The whole driver is inside `if (profileOn && !profileHasLowerTier && !profileMulti)`, so profile-absent / multi-tier / multi-format ⇒ it NEVER runs ⇒ `out`/zip byte-identical to today. Source mime === target ⇒ no-op (no skip noise). Identical pixel dims ⇒ NO `vramSaved` (inv 5; `exportProfile.assets` is a DISK file count). Decision tested in `export-profile-fanout.test.ts` + `atlas-profile-force.test.ts`; pixels verified manually (B2).
